...
public class AddProfileDK extends AppCompatActivity {
    ...
    private void NevigateMainProfile() {
        binding.btnContinue.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (newName.isEmpty()) {
                    Toast.makeText(AddProfileDK.this, "Vui lòng nhập họ và tên", Toast.LENGTH_SHORT).show();
                    return;
                }

                if (TextUtils.isEmpty(newDOB)) {
                    Toast.makeText(AddProfileDK.this, "Vui lòng nhập ngày sinh", Toast.LENGTH_SHORT).show();
                    return;
                }
                ...
                try {
                    ...
                    Toast.makeText(AddProfileDK.this, "Định dạng ngày sinh không đúng (dd/mm/yyyy)", Toast.LENGTH_SHORT).show();
                    return;
                }

                if (!newGender.equals("Nam") && !newGender.equals("Nữ")) {
                    // Nếu giới tính không phải 'Nam' hoặc 'Nữ', hiển thị thông báo và dừng lại
                    Toast.makeText(AddProfileDK.this, "Vui lòng chọn giới tính 'Nam' hoặc 'Nữ'", Toast.LENGTH_SHORT).show();
                    return;
                }

                if (!TextUtils.isDigitsOnly(newIDCard)) {
                    Toast.makeText(AddProfileDK.this, "CCCD phải là số", Toast.LENGTH_SHORT).show();
                    return;
                }

                if (TextUtils.isEmpty(newPhoneNumber)) {
                    Toast.makeText(AddProfileDK.this, "Vui lòng nhập số điện thoại", Toast.LENGTH_SHORT).show();
                    return;
                } else if (!TextUtils.isDigitsOnly(newPhoneNumber)) {
                    Toast.makeText(AddProfileDK.this, "Số điện thoại phải là số", Toast.LENGTH_SHORT).show();
                    return;
                }
                ...
                if (count > 0) {
                    ...
                    Toast.makeText(AddProfileDK.this, "Số điện thoại đã tồn tại", Toast.LENGTH_SHORT).show();
                    return;
                }
//                -----

                if (newAddress.isEmpty()) {
                    Toast.makeText(AddProfileDK.this, "Vui lòng nhập địa chỉ nơi ở", Toast.LENGTH_SHORT).show();
                    return;
                }
                ...
                if (rowId != -1) {
                    Toast.makeText(AddProfileDK.this, "Cập nhật thành công", Toast.LENGTH_SHORT).show();
                    Intent intent = new Intent(AddProfileDK.this, Chonhoso.class);
                    startActivity(intent);
                    intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_NEW_TASK);
                    finish();
                } else {
                    Toast.makeText(AddProfileDK.this, "Cập nhật không thành công", Toast.LENGTH_SHORT).show();
                }
                finish();
            }
        });
